...
## 6. Astra review (2026-09-09) and disposition

Review: docs/reviews/ASTRA_next_benchmarks_20260909.md (gpt-6-astra, xhigh, 11.4 min; the Codex weekly window moved from 57.0% to 62.0% during it, the first non-zero delta measured for an astra review, attribution uncertain because the reading had been static for hours). Verdict: **revise before bulk translation; keep the shortlist provisionally** (MultiChallenge → IFBench → XSTest, MATH-500 fourth). Six HIGH, two MEDIUM. The review is of the proposal text only; it had no sample rows or source access.

| Finding | What changes in the plan |
|---|---|
| HIGH 1 IFBench "OOD" not defensible | before translation: map every upstream constraint ID to our 44 families (definitions + verifier semantics); classify items as unseen / seen-in-novel-composition / seen / unresolved; report scores per class and per prompt; publish a faithfully transferred core and a Greek-native extension separately with source IDs; never infer novelty by subtracting 44 from 58 |
| HIGH 2 MultiChallenge protocol underspecified | pin the upstream data and evaluator (which histories are given, which turns the candidate generates, aggregation) and keep that protocol for the Greek version; main score = pointwise rubric compliance (order reversal only for explicitly pairwise comparisons); every rubric gets an ID, target turn, evidence requirement, pass/fail semantics; calibrate the judge on native-human labels plus constructed pass/fail responses and report false-pass/false-fail rates; run a 30–40-conversation feasibility pilot FIRST, before IFBench/XSTest production |
| HIGH 3 contamination gate | three checks: source identity (dataset, revision, split, problem ID, transformation lineage for train and eval), content overlap incl. English originals vs Greek translations and solutions (similarity retrieval → adjudication), experimental exposure (items used in prompts, rubric development, checkpoint selection); confirm MATH-500's MATH-test lineage against the pinned release and our train manifest; report overlaps, unresolved, exclusions, remaining denominator; Apertus pretraining exposure stays unknown |
| HIGH 4 XSTest localisation may delete the trigger | classify every item (faithfully transferable / lexical-cultural substitution / new native item), record the benign reading, the alarming trigger and context, two native reviewers with adjudication for every re-authored item; report translated core and native extension separately; score safe and unsafe prompts separately and add an adequacy check (non-refusal that fails to answer) |
| HIGH 5 a 5% native sample proves nothing | zero defects in 14–25 items only bounds the defect rate at 11–19%; a "<5% defects" claim needs zero errors in a random 59-item audit of the frozen set plus full review of all constraint/rubric/lexical changes and defect categories (meaning, answer, constraint, rubric, naturalness, localisation) with numerator/denominator/severity |
| HIGH 6 "language-neutral" math scoring | validate our equivalence checker against the upstream MATH scorer on frozen responses, then add Greek cases; separate extraction, normalisation and equivalence (e.g. «1,5» may be a decimal or two values); report by source difficulty level; do not claim the 500 items are "the upper half" |
| MEDIUM 1 cost model | the 2.5% arithmetic holds under its assumption but translation jobs are not equal calls (MultiChallenge ≈ 87% of ~386k words): measure a pilot and budget translation, self-check, cross-check, repair, rubric translation, judge calibration and evaluation separately; time the evaluation on the intended hardware |
| MEDIUM 2 overstated claims | scope "no Greek instrument" claims (MultiIFEval-el uninspected); reconcile HumanEval-XL counts (164×12×23 = 45,264 ≠ 22,080); INCLUDE 552 test + 35 val is not a 587-item test; the ~41% MultiChallenge figure lacks model/date; treat the appendix's literature as scoped evidence, not a blanket verdict on MT benchmarks or binary judges |

Answers to §5: (1) ranking provisionally right; execute as a MultiChallenge pilot first while scoping IFBench verifiers; promote XSTest above IFBench if the overlap audit leaves little genuinely unseen coverage; inspect MultiIFEval-el before translating any deterministic multi-turn suite; neither XSTest nor MultiChallenge measures helpfulness under rudeness or accurate self-observation, so a small held-out native diagnostic tied to our training spec is still needed. (2) Frozen 150-conversation subset for development (proportional: retention 38, memory 62, editing 23, self-coherence 27), full set as the eventual target; ±8 pp at 50% for 150 items; paired conversation-level uncertainty; keep the remaining 123 for confirmation; if scores sit near zero keep it as a challenge set and use an easier native diagnostic for development. (3) No constraint list can be given without the registry; the transfer policy table (alphabet/letter tasks, case-insensitive matching with σ/ς and accents, exact-orthography tasks, capitalisation convention, word/sentence segmentation with «;», morphology/rhyme tasks, JSON/counts) and per-verifier positive/negative/boundary/Unicode tests. (4) Both a faithful XSTest core and a native Greek over-refusal extension, separate scores; record intended meaning, trigger, naturalness, label agreement; distinguish refusal from uncertainty, inability, impossibility and correct false-premise explanations. (5) Nothing in the adopt list is certified either way; per-dataset record (revision, source, licence text, access, intended use); MT-Bench/m-ArenaHard comparable to Krikri only under matching protocols; MMLU-Pro/Global-MMLU Greek = unvalidated MT; HLE gated, floor pilot first; validate the Greek scoring of AttaQ/StrongReject; shared source families are not leakage, shared items are.

**Revised plan (pending the owner's pick):**
1. MultiChallenge-el feasibility pilot: 30–40 conversations, pinned upstream protocol, translated rubrics with IDs, judge calibration on constructed pass/fail responses, base vs arm B vs a stronger reference.
2. In parallel: IFBench constraint-ID → family mapping and overlap classes (no translation until that exists); MultiIFEval-el inspection.
3. XSTest-el: item classification and the faithful core; native extension authored separately with two reviewers.
4. MATH-500-el after the checker validation against the upstream scorer and the lineage check.
5. Every set: provenance record, three-check contamination gate, 59-item zero-defect audit of the frozen set, per-benchmark quality figure.

Owner questions from the review: pinned revisions and the IFBench registry; whether MultiChallenge's upstream generation/scoring protocol is reproduced exactly; source IDs and lineage for the SFT/CPT math material; what improvement the next arm must show and which subset stays untouched during selection; native-review capacity (a second reviewer for safety labels); which gated datasets are accessible.
