...
**Reconciliation update:** local Krikri micro is 52.0082%; its 31-subject macro is 53.6175%. Pinned official code scores Greek labels under a subject-specific prompt, whereas our frozen scorer uses full-answer average token likelihood under a generic prompt. Preserve the old score as a custom diagnostic. A 200-item subset has been selected independently of model correctness for the two-protocol comparison. See [code evidence and next measurement](execution/wave2/greekmmlu_protocol/report.md).
