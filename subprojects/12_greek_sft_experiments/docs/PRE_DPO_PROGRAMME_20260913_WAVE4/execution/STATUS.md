# Pre-DPO execution status

## Current programme status — 13 September, 13:43 UTC

The goal remains active. No new training or corpus promotion has occurred. Maths generation completed 88 successful jobs in 92 attempts; full problem review accepts eight rebuilt reused solutions and eight retained controls, with sixteen fresh Level 5 examples undergoing final assembly after two source-question repairs. Seven separate trained-maths repairs and twenty IF candidates already passed independent review.

The four-decision balanced-dialogue probe failed its semantic gate despite valid structured responses. Its next revision must bind labels to visible evidence, preserve the user’s exact decision and distinguish text edits from external actions. All four personality deltas passed independent review. The32-row maths package also passed exact tokenization and decoded-control checks; its16fresh examples are receiving a bounded Greek-language check.

The first85-minute debug-node peer measurement is running as job3390957, start13:44:05Z, with a maximum estimated CHF3.81 inside the existing CHF10.76 measurement envelope. Actual usage remains pending. Current ledger baseline is CHF 163.02 used / CHF 66.98 headroom under the CHF 230 cumulative SFT cap. The account’s last user-provided overall balance remains CHF 2,095.08.

See [decisions](../DECISIONS.md), [current plan](../PLAN.md) and [wave state](wave2/STATE.json). The following is the preserved first-wave report; its status and budget observations are historical.
