...
def nonassistant_call_quarantine(turn: dict):
    """Return an explicit quarantine reason for truthy calls on non-assistant turns."""
    raw = turn.get("function_calls")
    if not raw or turn.get("role") == "assistant":
        return None
    if isinstance(raw, str):
        try:
            ...
            parsed = object()
        if parsed is None:
            return "nonassistant_function_calls:null_string_sentinel"
    return f"nonassistant_function_calls:role_{turn.get('role')}"
    ...
    for ordinal in range(contract["target_rows"]):
        ...
        for original, exported in zip(msgs, roundtrip["turns"]):
            ...
            if original.get("function_calls"):
                quarantine = nonassistant_call_quarantine(original)
                if quarantine:
                    protocol_errors.append(quarantine)
                try:
