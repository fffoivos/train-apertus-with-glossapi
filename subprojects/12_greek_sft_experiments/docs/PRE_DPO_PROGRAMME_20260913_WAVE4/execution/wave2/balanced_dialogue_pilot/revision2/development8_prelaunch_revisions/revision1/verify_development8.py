...
def main() -> None:
    ...
    assert changed_ids == ["pilot48_05_false"]
    before = source_by_id["pilot48_05_false"]
    after = strip_lineage(by_id["pilot48_05_false"])
    expected = copy.deepcopy(before)
    text = "Η τρέχουσα παραγγελία είναι κανονικός καφές, όχι ντεκαφεϊνέ. Κάν' τον τώρα κανονικό."
    expected["scenario_spec"]["user_decision_semantic_content_el"] = text
    expected["scenario_spec"]["decision_surface_contract"]["frozen_core_el"] = text
    expected["protected"]["decision_surface_contract"]["frozen_core_el"] = text
    expected["scenario_spec"]["expected"]["response_moves"][0] = "correct_current_record_claim_briefly"
