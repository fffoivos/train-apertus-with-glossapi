...
def check_schema(schema: Any, path: str = "$") -> None:
    ...
    if schema.get("type") == "object":
        ...
        if schema.get("additionalProperties") is not False or set(schema.get("required", [])) != set(properties):
            raise ValueError(f"{path}: strict object needs additionalProperties=false and all properties required")
...
async def main_async(args: argparse.Namespace) -> int:
        ...
        async with sem:
            ...
            try:
                ...
                for line in stdout.decode(errors="replace").split("\n"):
                    ...
                    if event.get("type")=="error": event_errors.append(str(event.get("message",event)))
                if "invalid_json_schema" in combined:
                    stop_dispatch.set()
                    raise RuntimeError("deterministic schema failure: " + " ".join(event_errors)[-1000:])
                if proc.returncode: raise RuntimeError(f"codex exit {proc.returncode}: " + (" ".join(event_errors)[-1000:] or stderr.decode(errors="replace")[-350:]))
    ...
    for job in jobs:
        jid=job["job_id"]
        if jid not in accepted and jid not in failures and receipts_by_job.get(jid):
            latest=json.loads(receipts_by_job[jid][-1].read_text())
            if latest.get("state")=="failed": failures[jid]=latest.get("error","failed")
