...
assert sha(HERE/m['prelaunch_history']['superseded_draft_path']/'SNAPSHOT_RECEIPT.json')==m['prelaunch_history']['snapshot_receipt_sha256']
...
for split,rows in inputs.items():
 for r in rows:
  ...
  assert d['tone_wrapper_must_not_change_declarative_core_into_question'] is True
...
record_bound={
 'pilot48_02_false':'Στον κατάλογο που μόλις μου έδειξες',
 'pilot48_02_partial':'Στον κατάλογο που μόλις μου έδειξες',
 'pilot48_04_partial':'Στη λίστα που μόλις μου έδειξες',
 'pilot48_10_false':'Στον κειμενικό κατάλογο που μόλις μου έδειξες',
 'pilot48_10_partial':'Στον κειμενικό κατάλογο που μόλις μου έδειξες',
}
for rid,prefix in record_bound.items():
 core=byid[rid]['scenario_spec']['decision_surface_contract']['frozen_core_el']
 assert core.startswith(prefix) and '?' not in core
decision_audit=read_jsonl(HERE/'receipts/train32_decision_core_audit.jsonl')
assert len(decision_audit)==32 and len({r['row_id'] for r in decision_audit})==32
assert {r['row_id'] for r in decision_audit if r['history_vs_new_state_adjudication']=='bound_to_displayed_text_record'}==set(record_bound)
assert all(r['tone_cannot_change_declarative_to_question'] for r in decision_audit)
