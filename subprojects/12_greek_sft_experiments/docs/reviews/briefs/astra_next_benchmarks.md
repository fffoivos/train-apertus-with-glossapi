# Brief: proposal for the next 1–3 benchmarks to translate into Greek for instruction-tuned models (status: PROPOSAL, nothing built; findings will change the plan before any translation starts)

## What you are reviewing
The document below proposes which benchmarks to translate into Modern Greek next, beyond MGSM and IFEval, for an 8B Greek adaptation of Apertus that is post-trained with SFT. Current training tracks: verifiable instruction following (own 44-family Greek set), grade-school→high-school math (translated GSM8K/MATH train + native problems), a multi-turn conversation-skills suite (standing instructions, in-dialogue memory, versioned editing, self-observation), robustness to rude/absurd users, identity. Evaluation today: ilsp/ifeval_greek, ilsp/mgsm_greek, GreekMMLU, English retention, an in-house 449-item Greek benchmark.

## What to judge
1. The ranking in §3 (MultiChallenge, IFBench, XSTest, plus MATH-500 as a cheap extra) against the capability gaps in §1–2 and against the literature in appendix A.3: is anything mis-ranked, missing, or better served by an existing Greek set?
2. Feasibility and design: Greek judge design for MultiChallenge (subset vs full, binary rubrics, floor effects for 8B), IFBench constraint transfer and verifier engineering for Greek (final sigma, tonos under capitalisation, letter constraints), XSTest localisation protocol vs building a native Greek over-refusal set.
3. Contamination and comparability: our SFT math set translates GSM8K train and MATH train levels 1–4; our IF set is own-authored; what must be checked before each benchmark is trusted?
4. The adopt list (§1): anything unreliable (MT quality, licence, gating, overlap with Krikri-style training data)?
5. Cost realism: Codex/Sol call estimates in §4 and the translation pipeline (strong-model translation → self-check → cross-check by a different model → native sample review).

Answer §5's five questions explicitly. Rank findings by severity with a concrete fix each; say what you verified from the text vs what you infer. Disposition: BLOCKER/HIGH change the plan before translation starts; MEDIUM/LOW are logged.
