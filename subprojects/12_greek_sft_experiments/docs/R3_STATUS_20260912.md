...
| IFBench-el prompt strict (unseen constraint types) | 9.3 | 9.3 | pending | | flat: the IF gain did not transfer |
| MATH-500-el equiv500 | 8.0 | 7.6 | pending | | flat; better at levels 2–3, worse at 4 |
...
| H4 | loops are a property of the single-stage schedule that arm B's pass removed (stage 1 loops too) | unknown | the four benchmarks on stage 1 under the same greedy protocol (running, debug window; loop and truncation counts) | running |
