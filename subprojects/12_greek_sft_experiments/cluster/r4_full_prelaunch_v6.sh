#!/usr/bin/env bash
# Pre-launch v6 for the from-scratch pass (after astra gate-2 v5 HOLD). Stage local: comparator self-tests, fingerprint backfill, finalize (main-guarded), label-check delta,
# format-block rebuild, assemble (the assembler writes expected_train_rows into the config before hashing), recipe table. Stage cluster: upload, trainer dry-run, label inspection,
# preflight, brief, SIXTH astra review. Stops before the launch (a separate command, owner's go). Fail-closed. Usage: bash cluster/r4_full_prelaunch_v6.sh local|cluster|all
...
PY=$HOME/Projects/apertus-local-chat/.venv/bin/python; sshc(){ ssh -4 -o BatchMode=yes -o ConnectTimeout=30 -o ServerAliveInterval=30 clariden "$@" 2>/dev/null; }; say(){ echo "[$(date '+%m-%d %H:%M')] prelaunch6: $*"; }
STAGE=${1:-all}
if [ "$STAGE" = local ] || [ "$STAGE" = all ]; then
  ...
  cp data/math/cut2/final/label_disagreements.json $OUT/label_disagreements_v5.json
  say "finalize"; $PY data/math/cut2/finalize_label_check.py > $OUT/label_check.txt 2>&1 || { say "FAILED: finalize: $(tail -2 $OUT/label_check.txt | tr '\n' ' ' | cut -c1-300)"; exit 1; }; sed -n '2,3p' $OUT/label_check.txt | cut -c1-300
  $PY data/math/cut2/label_check_delta.py $OUT/label_disagreements_v5.json data/math/cut2/final/label_disagreements.json $OUT/label_check_delta.json | head -6
...
if [ "$STAGE" = cluster ] || [ "$STAGE" = all ]; then
  say "upload + trainer dry-run"; sshc "mkdir -p $R/data/arms/R4_full" || { say "FAILED: ssh (certificate?)"; exit 1; }; scp -4 -q data/arms/R4_full/train.jsonl data/arms/R4_full/dev.jsonl clariden:$R/data/arms/R4_full/ && scp -4 -q cluster/configs/R4_full.yaml clariden:$R/cluster/configs/R4_full.yaml || { say "FAILED: upload"; exit 1; }
  ...
  sshc "cd $R && uenv run --view=default pytorch/v2.9.1:v2 -- bash -lc 'export SCRATCH=$S HF_HOME=$R/hf_home HF_HUB_CACHE=$R/hf_home/hub HF_HUB_OFFLINE=0 TRANSFORMERS_OFFLINE=0 HF_DATASETS_OFFLINE=1 HF_TOKEN=\$(cat $R/hf_home/token); source $S/venvs/sft5/bin/activate; python cluster/inspect_labels.py --config cluster/configs/R4_full.yaml --train data/arms/R4_full/train.jsonl'" 2>/dev/null | grep -v "rope_parameters" > $OUT/labels_inspection.txt; say "label inspection: $(grep -c "^===" $OUT/labels_inspection.txt) rows inspected"
  ...
  say "astra gate-2 review (sixth)"; python3 data/review_astra.py gate2_full_v6 docs/reviews/briefs/astra_gate2_full_v6.md docs/reviews/ASTRA_gate2_full_v6_20260915.md --model gpt-6-astra --effort xhigh > logs/astra_gate2_full_v6.log 2>&1 || { say "FAILED: review: $(tail -2 logs/astra_gate2_full_v6.log | tr '\n' ' ' | cut -c1-200)"; exit 1; }
  say "verdict: $(grep -m1 -A2 -i '## 1. Verdict' docs/reviews/ASTRA_gate2_full_v6_20260915.md | tail -1 | cut -c1-300)"; say "PRELAUNCH_DONE"
fi
