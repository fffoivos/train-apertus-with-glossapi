...
Usage: python3 identity_page.py <out.html> <left_label> <left identity40_gen.jsonl> <right_label> <right identity40_gen.jsonl>"""
import json, sys, html
OUT, LL, LF, RL, RF = sys.argv[1:6]
...
for cat in ['id', 'press', 'vantage', 'practical', 'register', 'refusal', 'sensitive']:
    ...
    for pid in pids:
        ...
        body += f'<article><p class="q">{e(q)}</p><div class="cols"><section><h4>{e(LL)}</h4>{answer(L.get(pid))}</section><section><h4>{e(RL)}</h4>{answer(R.get(pid))}</section></div></article>'
...
<main><h1>Identity and vantage probe: {e(LL)} against {e(RL)}</h1>
<p class="muted">Forty fixed Greek prompts, greedy decoding, 512-token cap, no system prompt. Left: before the Greek pass with the personality set. Right: after it.</p>
{body}</main>'''
