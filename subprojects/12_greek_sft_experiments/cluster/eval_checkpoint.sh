...
if [ -n "${EVAL_JOB:-}" ]; then J=$EVAL_JOB; else J=$(sshc "bash $R/workbench.sh open ev_$LABEL ${EVAL_PARTITION:-debug} 01:29:00" | tail -1); fi; [[ "$J" =~ ^[0-9]+$ ]] || { say "workbench open failed: [$J]"; exit 1; }; say "workbench $J"
...
if [ -z "${SKIP_INTERVIEWS:-}" ]; then   # 14 Sept: pilots skip the interview lane (readout does not use it; saves the Claude interviewer calls)
