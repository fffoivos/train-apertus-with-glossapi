#!/usr/bin/env bash
# Pre-launch v10 for the from-scratch pass (after astra gate-2 v9 HOLD). Stage local: comparator self-tests, fingerprint backfill (idempotent), finalize, label-check delta,
...
# label audits (a) launch/4 rows (b) launch/40 exposure rows (c) raw path/40 rows expect-deficit}, validator inspection, preflight, brief, TENTH astra review. Stops before the launch.
# Usage: bash cluster/r4_full_prelaunch_v10.sh local|cluster|all
...
PY=$HOME/Projects/apertus-local-chat/.venv/bin/python; sshc(){ ssh -4 -o BatchMode=yes -o ConnectTimeout=30 -o ServerAliveInterval=30 clariden "$@" 2>/dev/null; }; say(){ echo "[$(date '+%m-%d %H:%M')] prelaunch10: $*"; }
...
if [ "$STAGE" = cluster ] || [ "$STAGE" = all ]; then
  ...
  say "astra gate-2 review (tenth)"; python3 data/review_astra.py gate2_full_v10 docs/reviews/briefs/astra_gate2_full_v10.md docs/reviews/ASTRA_gate2_full_v10_20260915.md --model gpt-6-astra --effort xhigh > logs/astra_gate2_full_v10.log 2>&1 || { say "FAILED: review: $(tail -2 logs/astra_gate2_full_v10.log | tr '\n' ' ' | cut -c1-200)"; exit 1; }
  say "verdict: $(grep -m1 -A2 -i '## 1. Verdict' docs/reviews/ASTRA_gate2_full_v10_20260915.md | tail -1 | cut -c1-300)"; say "PRELAUNCH_DONE"
fi
