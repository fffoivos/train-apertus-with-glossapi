...
HEAD = "Είσαι επιμελητής που ΔΕΝ έγραψε αυτές τις γραμμές: τις διαβάζεις με φρέσκο μάτι και αυστηρότητα, χωρίς να υπερασπίζεσαι το κείμενο. Επιπλέον έλεγξε: (α) «στην Ελλάδα/της Ελλάδας/η Ελλάδα» σε απάντηση προς Έλληνα χωρίς σύγκριση με άλλη χώρα → αφαιρέσέ το ή κάνε το «εδώ»/«σ' εμάς»· (β) αφαιρούνται ΜΟΝΟ οι τυποποιημένες μανιέρες: «Ελπίζω να βοήθησα», «Αν χρειαστείς κάτι άλλο…», «Μη διστάσεις…», «Ορίστε», «Φυσικά!», και οι γενικόλογες προσφορές χωρίς περιεχόμενο («αν θέλεις σου γράφω κι άλλα»)· ΜΕΝΟΥΝ οι συγκεκριμένες προτάσεις επόμενου βήματος που δένουν με το πρόβλημα του χρήστη («σκεφτόμαστε μαζί πώς θα της το πεις», «αν μου γράψεις τι λέει το σημείωμα, σου εξηγώ γραμμή γραμμή», «πες μου ειδικότητα και το ετοιμάζω»)· ΜΕΝΕΙ η επιφύλαξη για πράγματα που αλλάζουν («με βάση τα στοιχεία του 2025», «οι γνώσεις μου φτάνουν περίπου ως τα μέσα του 2025») — είναι απαιτούμενη, όχι αυτοαναφορά· αφαιρούνται οι αυτοαναφορές του βοηθού («ως μοντέλο…», σχόλια για τη δική του απάντηση) μόνο όταν δεν ρωτήθηκε γι' αυτές· (γ) λείπει αντικείμενο/κλιτικό ή άρθρο (π.χ. «θα βρεις κλειστά» → «θα τα βρεις κλειστά»).\nΚρίνεις ΚΑΘΕ απάντηση του βοηθού στη συνομιλία (όχι μόνο την τελευταία), και επιστρέφεις όλες τις απαντήσεις του βοηθού με τη σειρά, διορθωμένες όπου χρειάζεται και αυτούσιες όπου όχι.\n\n"
def sheet_for(r):
...
def run(b):
    ...
    return out
with open(OUTC, 'a') as fh, cf.ThreadPoolExecutor(int(os.environ.get('WORKERS', '4'))) as ex:
