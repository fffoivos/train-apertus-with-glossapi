...
Usage: python3 data/assemble_stage2.py [replay_frac=0.10] [seed=2026] [personality_weight=1] [arm=R2_stage2]  → data/arms/R2_stage2/{train,dev}.jsonl + summary.md
...
PW = int(sys.argv[3]) if len(sys.argv) > 3 else 1; ARM = sys.argv[4] if len(sys.argv) > 4 else 'R2_stage2'   # personality weight (rows repeated PW times), output arm name
rng = random.Random(seed); A = 'data/arms/R2_stage1'; O = f'data/arms/{ARM}'; os.makedirs(O, exist_ok=True)
...
train += replay + [P(r) for r in pers_train for _ in range(PW)]; rng.shuffle(train)
