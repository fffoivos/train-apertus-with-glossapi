...
PER_FORM=${PER_FORM:-2}; EXCLUDE=${EXCLUDE:-}; LAYOUT=${LAYOUT:-0.55,0.25}; SEED=${SEED:-2026}   # v2: PER_FORM=3 EXCLUDE=v1/prompts.jsonl LAYOUT=0.75,0.10 SEED=2027
log(){ echo "$(date '+%F %T') $*"; }
...
[ -s "$RUN/request_bank.jsonl" ] && [ "$(wc -l < "$RUN/request_bank.jsonl")" -ge $((N + 2000)) ] || { log "stage 2: request bank"; python3 gen_requests.py "$RUN/request_bank.jsonl" "$PER_FORM" || exit 1; }
[ -s "$RUN/prompts.jsonl" ] || { log "stage 3: prompts"; python3 gen_prompts.py "$RUN/prompts.jsonl" --n "$N" --seed "$SEED" --level-weights .25,.30,.25,.12,.08 --families "$CHECKABLE" --requests "$RUN/request_bank.jsonl" ${EXCLUDE:+--exclude "$EXCLUDE"} --layout "$LAYOUT" || exit 1; }
