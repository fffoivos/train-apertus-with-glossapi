...
summ = dict(sft_rows=len(sft), dpo_pairs=len(dpo), exact_duplicates_dropped=dup, shared_12word_prefix_share=round(shared, 4), by_version=dict(ver), by_level=dict(sorted(lvl.items())), by_form=dict(form), families=len(fam), min_family_rows=min(fam.values()), max_family_rows=max(fam.values()),
