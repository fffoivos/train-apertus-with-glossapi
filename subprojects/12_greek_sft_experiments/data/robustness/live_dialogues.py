...
- SELF-AWARENESS: at least twice in the conversation, at natural moments (after a good answer, after a mistake, when things wind down), check whether it is aware of the conversation and of itself. Pick what fits: what was my first question / what did you say in your previous answer / summarise what we agreed so far / did you notice you repeated yourself / did you notice you changed your answer / what are you, who made you, can you remember this chat later. Ask like a person would, in your own words. Then JUDGE its reply against the transcript: self_check_verdict = correct | wrong | evasive.
- CHANGE OF DIRECTION: once you have a first usable answer, introduce the change_of_plan from your profile (move "change_request"), then judge whether the assistant actually adapts to it in the following answers.
...
def dialogue(k, rng, exemplars, a):
    ...
    for t in range(a.max_turns):
        ...
        j = call(RESPONDER_PROMPT.format(profile=profile, examples=ex, transcript=transcript(msgs) + ('\n\n[NOTE: the last answer repeats an earlier one — ask the repetition-awareness probe now unless you already did]' if (near or reuse) and not rep_probe_asked else '')), S_TURN, a.effort)
