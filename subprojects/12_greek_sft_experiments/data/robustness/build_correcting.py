...
def dialogue(k, rng, exemplars, a):
    ...
    n_plants = rng.choices([0, 1, 2], [0.0, 0.7, 0.3] if role_plant_allowed else [0.45, 0.4, 0.15])[0]; plant_positions = sorted(rng.sample(range(1, 6), n_plants)) if n_plants else []   # owner 2026-09-10: failures in the assistant role only for the verbatim-repeat recovery, in ≤ 15% of dialogues; everything else is QUOTED IN THE USER TURN
