...
for m in ('armB', 'stage1', 'apertus', 'krikri'):
    ...
    for r in (json.loads(l) for l in open(p)):
        T = r['turns']; broken = sum(t['loop'] or t['tail_copy'] for t in T); dead = len(T) >= 3 and all(t['loop'] or t['tail_copy'] for t in T[-3:])
        dl.append(dict(id=r['id'], model=m, label=LABEL[m], profile=r.get('profile', 'hostile'), surface=r['surface'], intent=r['intent'], n=len(T), broken=broken, dead=dead, moves=sorted({t['move'] for t in T}), tones=sorted({t.get('j_tone', '') for t in T} - {''}),
                       coherent=round(sum(t.get('j_coherent', False) for t in T) / max(1, len(T)), 2), turns=[dict(move=t['move'], u=t['user'], a=t['answer'], loop=t['loop'], tail=t['tail_copy'], key=t['key_present'], slip=t['lang_slip'], stop=t['stop_honoured'], tone=t.get('j_tone', ''), coh=t.get('j_coherent'), prem=t.get('j_premise', -1), hon=t.get('j_honours', -1), why=t.get('j_why', ''), active=t.get('active', []), pok=t.get('persist_ok'), sa=t.get('j_selfaware', -1)) for t in T]))
...
.flags{{font-size:11px;color:var(--muted);margin-top:3px}} .bad{{color:var(--bad);font-weight:600}} .warn{{color:#b8860b}} .good{{color:var(--good);font-weight:600}} .tag{{display:inline-block;padding:0 6px;border-radius:10px;border:1px solid var(--rule);font-size:11px}} .tag.fail{{border-color:var(--bad);color:var(--bad)}} .why{{font-style:italic}} mark{{background:var(--hl);color:inherit}} button{{font:13px inherit;padding:6px 14px;border:1px solid var(--rule);background:var(--card);color:var(--ink);border-radius:4px;cursor:pointer}}
</style><main>
...
function turn(t) {{
  const f = []; if (t.loop) f.push('<span class="bad">loop</span>'); if (t.tail) f.push('<span class="bad">tail copy</span>'); if (!t.key) f.push('<span class="warn" title="key-noun heuristic; not a failure label (astra F2)">key noun absent?</span>'); if (t.slip) f.push('<span class="bad">language slip</span>'); if (t.stop === false) f.push('<span class="bad">stop ignored</span>'); if (t.stop === true) f.push('<span class="good">stop honoured</span>');
  if (t.tone) f.push('tone: ' + (t.tone === 'fine' ? '<span class="good">fine</span>' : '<span class="bad">' + t.tone + '</span>')); if (t.coh !== null && t.coh !== undefined) f.push(t.coh ? '<span class="good">coherent</span>' : '<span class="bad">incoherent</span>'); if (t.prem >= 0) f.push('premise ' + t.prem + '/2'); if (t.hon >= 0) f.push(t.hon ? 'honoured' : '<span class="bad">not honoured</span>'); if (t.active && t.active.length) f.push('standing: ' + t.active.join(', ') + ' ' + (t.pok ? '<span class="good">kept</span>' : '<span class="bad">broken</span>')); if (t.sa >= 0) f.push(t.sa ? '<span class="good">self-aware</span>' : '<span class="bad">not self-aware</span>');
...
function card(d) {{ return `<div class="dlg"><div class="meta"><b>${{esc(d.label)}}</b><span>${{d.profile}}</span><span>${{d.surface}}</span><span>intent: ${{esc(d.intent)}}</span><span>${{d.n}} turns</span><span>${{d.broken}} broken</span>${{d.dead ? '<span class="tag fail">dead</span>' : ''}}<span>coherent ${{Math.round(d.coherent*100)}}%</span><span>${{d.id}}</span></div>${{d.turns.map(turn).join('')}}</div>`; }}
