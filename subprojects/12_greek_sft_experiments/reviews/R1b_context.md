# R1b — re-certification of the two R1 fixes (F1 twin-dev leak, F2 batch unit)
...
## STEP FOCUS (R1b)
This is the re-review after R1 (`reviews/R1_data_trainer.md`, read it). Scope = the fix commit `a4b0005c` (`git show a4b0005c`). Verify firsthand: (1) F1 — `data/splits/no_robots_en_pov.dev.txt` equals `data/splits/no_robots.dev.txt`; in `data/arms/E2/train.jsonl` and every arm no train row shares a row_id with any `dev_all.jsonl` row of either twin config; recompute E2 tokens (claimed 9,360,153); (2) F2 — every production config and `cluster/configs/zero3.yaml` carry `gradient_accumulation_steps: 4` (16 packed sequences/step), `smoke.yaml` is 30 steps; the dry-run plan for `E1_lr1e-5_3ep_const.yaml` reports ~95 steps/epoch and ~285 planned steps with ~9 warmup; the decision is recorded in `SFT_PLAN_20260903.md` §10 amendment 2 and `execution_state.json`. Report anything the fixes broke. Do not re-litigate F3–F6 (logged LOW).
