...
| M1 | E1 and E2 each drew *"an online application keeps rejecting a correctly formatted address"* for el / `plan_or_organise` and rendered near-paraphrases at **Jaccard 0.000**. 17 distinct situations for 20 seeds. | **Fixed.** A situation is not issued twice for the same language + subtype. |
