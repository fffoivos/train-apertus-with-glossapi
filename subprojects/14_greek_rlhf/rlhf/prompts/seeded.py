...
def generate_into(bank, plan_id, kind, n, *, run, workdir, legacy_registry, prior_runs, fake=False, workers=8, max_rounds=4, python=sys.executable):
    ...
    made, held, skipped, calls = [], [], collections.Counter(), collections.Counter()
    for rnd in range(1, max_rounds + 1):
        ...
        if p.returncode != 0:
            for s in batch: bank.release(s["source_id"])          # the generator died: the slots are unused, not spent
    ...
    return {"registered_active": len(made), "registered_held": len(held), "prompt_ids": made, "held_ids": held,
            "skipped": dict(skipped), "sol_calls": dict(calls), "asked_for": n}
