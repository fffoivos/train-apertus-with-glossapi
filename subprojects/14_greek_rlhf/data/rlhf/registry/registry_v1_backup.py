...
def joint_targets(N):
    """Integer targets per purpose x language cell: purpose margins by largest remainder, then languages within each purpose by largest remainder."""
    def lr(total, shares):
        ...
        return base
    pm = lr(N, TARGET['primary_purpose_shares_percent']); T = {}
    for p in PURPOSES:
        lm = lr(pm[p], TARGET['language_shares_percent'])
        for l in LANGS: T[(p, l)] = lm[l]
    return T
...
def report(c, sizes):
    ...
    for lid, p, l, elig, reason, kind in c.execute('SELECT p.logical_id, p.primary_purpose, p.language, pc.eligible, pc.reason, p.source_kind FROM prompts p JOIN pair_credit pc USING(logical_id) WHERE p.keepable=1'):
        E[(p, l)] += elig; judged[(p, l)] += 1; judged_elig[(p, l)] += 1 if (elig or reason.startswith('held')) else 0
    # yield: pairs meeting the pair rule per judged keepable prompt (holds count as meeting the rule: the rule was met before the hold)
    y = {}
    ...
    for p in PURPOSES:
        for l in LANGS:
            ...
            if n >= 10: y[(p, l)] = dict(estimate=k / n, low=wilson(k, n)[0], high=wilson(k, n)[1], basis=f'measured {k}/{n}')
    ...
    for url, forum, payload, status in c.execute("SELECT url, forum, payload, status FROM sources WHERE kind='forum_post'"):
        ...
        if rl and rl['maths_content'] and p != 'math': continue   # maths content inside a non-maths forum post: held from A until the specialist judge
        if p: A[(p, 'el')] += 1; A_forum[forum] += 1
    out = dict(spec=str(SPEC.name), rubric_sha16=RUBRIC_SHA16, E=sum(E.values()), E_by_cell={f'{p}/{l}': E[(p, l)] for p in PURPOSES for l in LANGS if E[(p, l)]},
