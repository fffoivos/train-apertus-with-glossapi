#!/usr/bin/env bash
# Runs on the provisioned pod (dialogue v2 copy of dialogue_quality_depth/pod/dqd_pod_setup.sh, CK3/CK5-reviewed).
# Changes: pinned Hub revision, best-effort on-pod self-kill with sudo, DV2_* markers. Secret tracing is never enabled.
set -Eeuo pipefail
...
SELF_KILL_SECONDS="${DV2_SELF_KILL_SECONDS:-5400}"
MIN_FREE_GB=40
...
printf 'DV2_PREFLIGHT_OK work=%s free_gb=%s driver=%s cuda_track=%s gpu_memory_mb=%s vllm=%s\n' \
  "$WORK" "$free_gb" "$driver_version" "$cuda_track" "$gpu_memory_mb" "$vllm_version"

# Backstop against an orphaned pod: power off after SELF_KILL_SECONDS (best effort; the local watchdog is primary).
if sudo -n systemd-run --on-active="$SELF_KILL_SECONDS" /bin/sh -c 'poweroff' >/dev/null 2>&1; then
  printf 'DV2_SELF_KILL_ARMED seconds=%s\n' "$SELF_KILL_SECONDS"
else
  printf 'DV2_SELF_KILL_NOT_ARMED\n'
fi
...
"$WORK/vllmenv/bin/python" -c \
  "import torch,vllm; print('DV2_VLLM_OK version='+vllm.__version__+' torch='+torch.__version__+' cuda='+str(torch.version.cuda))" \
  || fail 45 "vllm_import"
...
HF_TOKEN="$hub_token" HF_HUB_ENABLE_HF_TRANSFER=0 \
  "$WORK/vllmenv/bin/hf" download "$MODEL_ID" --revision "$MODEL_REVISION" --local-dir "$MODEL_DIR" >/dev/null \
...
printf 'DV2_MODEL_SHA_OK sha256=%s\n' "$actual_sha"
...
printf 'DV2_MODEL_CONFIG_OK max_position_embeddings=%s\n' "$max_model_len"
...
for _ in $(seq 1 60); do
  if ! kill -0 "$server_pid" 2>/dev/null; then
    printf 'DV2_SETUP_FAIL code=48 reason=serve_process_died\n' >&2
    tail -20 "$WORK/serve.log" >&2 || true
    exit 48
  fi
  if curl -fsS --max-time 2 http://127.0.0.1:8000/v1/models 2>/dev/null | grep -Fq "$MODEL_ID"; then
    printf 'DV2_SERVE_READY model=%s max_model_len=%s pid=%s\n' "$MODEL_ID" "$max_model_len" "$server_pid"
    exit 0
  fi
  sleep 2
done
printf 'DV2_SETUP_FAIL code=49 reason=serve_ready_timeout\n' >&2
tail -20 "$WORK/serve.log" >&2 || true
exit 49
