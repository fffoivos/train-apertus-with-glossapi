...
def defs_for(slots, instance_stage=False):
    lines = {gl_line('Detail is defined by')}
    ...
    return '\n'.join(sorted(lines))
def main():
    ap = argparse.ArgumentParser(); ap.add_argument('--run', required=True); ap.add_argument('--retry-held', action='store_true'); ap.add_argument('--regenerate', default=''); ap.add_argument('--manifest', default=''); ap.add_argument('--slots', default=''); ap.add_argument('--first-n', type=int, default=0); ap.add_argument('--all-remaining', action='store_true'); ap.add_argument('--workers', type=int, default=24); ap.add_argument('--fake', action='store_true'); ap.add_argument('--registry', default=''); ap.add_argument('--runs-dir', default=''); a = ap.parse_args()
    global REG
    if a.registry: REG = pathlib.Path(a.registry)
    MANIFEST = pathlib.Path(a.manifest) if a.manifest else HERE / 'manifest_round3.json'
    man = json.load(open(MANIFEST)); runs = pathlib.Path(a.runs_dir) if a.runs_dir else HERE / 'runs'; out = runs / a.run; out.mkdir(parents=True, exist_ok=True)
    ...
    def call(stage, prompt, schema, effort='medium'):   # owner, 17 Sept: all pipeline calls run at medium effort
        calls[stage] += 1
    ...
    receipt = dict(run=a.run, slots=len(slots), active=len(final), held=len(slots) - len(final), repairs_used=collections.Counter(v[2] for v in final.values()), sol_calls=dict(calls), glossary_sha16=GL_SHA16,
                   code_sha16=hashlib.sha256((HERE / 'generate.py').read_bytes()).hexdigest()[:16], manifest=str(MANIFEST), manifest_sha16=hashlib.sha256(MANIFEST.read_bytes()).hexdigest()[:16], task_kinds_sha16=hashlib.sha256((HERE / 'task_kinds.json').read_bytes()).hexdigest()[:16], finished=time.time())
