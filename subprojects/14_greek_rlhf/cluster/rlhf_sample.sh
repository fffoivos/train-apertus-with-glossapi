...
hours=$(python3 -c "h,m,s='$WALL'.split(':'); print(int(h)+int(m)/60)"); bash ../12_greek_sft_experiments/cluster/preflight.sh $hours "rlhf_sample" | tail -1 | grep -q '^OK' || { say "FAILED: preflight refused"; exit 1; }
...
kill $TUN 2>/dev/null; sshc "bash $R/workbench.sh close $W" | tail -1 | tee -a $LOG; bash ../12_greek_sft_experiments/cluster/ledger.sh $W rlhf_sample "RLHF sampling window" | tail -1 | tee -a $LOG; say "RLHF_SAMPLE_DONE"
